...
my $imDir = "../../dataset/FDDB-folds/originalPics/"; 
# where the folds are
my $fddbDir = "../../dataset/FDDB-folds/"; 
# where the detections are
my $detDir = "../../dataset/FDDB-folds/FDDB_OUTPUT/"; 
###########################
...
sub makeGNUplotFile
{
  ...
  print GF "set term png size 1280, 1200 enhanced font 'Verdana,18'\n";
  ...
  print GF "plot \"$rocFile\" using 2:1 with linespoints pointinterval 50 lw 3 title \"V1.3\", \"$rocFileBaseline\" using 2:1 with linespoints pointinterval 50 lw 3 title \"baseline\"\n";
  close(GF);
}
...
#system($evaluateBin, "-a", $annotFile, "-d", $detFile, "-f", $detFormat, "-i", $imDir, "-l", $listFile, "-r", $detDir, "-s"); # will show the result for every pic
...
#makeGNUplotFile($detDir."baseline/DiscROC.txt", $gpFile, $detDir, $detDir."DiscROC.png");
#makeGNUplotFile($detDir."DiscROC.txt", $gpFile, $detDir, $detDir."DiscROC.png");
makeGNUplotFile($detDir."DiscROC.txt", $detDir."baseline/DiscROC.txt", $gpFile, $detDir, $detDir."DiscROC.png");
system("echo \"load '$gpFile'\" | $GNUPLOT");
